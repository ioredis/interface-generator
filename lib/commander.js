...
// Some argument names collide with JS/TS reserved words and cannot be used as
// parameter names. Remap them to a safe alternative.
const reservedWordReplacements = {
  function: "func",
};

const safeArgName = (name) => reservedWordReplacements[name] || name;
...
const processArguments = (
  ...
  // safeArgName is applied only where `name` becomes an argument parameter.
  // Token names (`${name}Token`) and type strings can never collide with
  // reserved words, so they keep the unmodified name.
  const argName = safeArgName(name);
  ...
  switch (type) {
    case "key":
      add([
        ...
        [
          ...
          { name: argName, type: typeMaps.key, multiple },
        ],
      ]);
      break;
    case "string":
    ...
    case "pattern":
      add([
        ...
        [
          tokenDef && { ...tokenDef, name: `${name}Token` },
          { name: argName, type: typeMaps.pattern, multiple },
        ],
      ]);
      break;
    case "integer":
...
const processCommand = (
  ...
  if (argumentTypes[def.name]) {
    ...
    const args = def.arguments.map(packObject);
    (sortArguments[def.name] ? sortArguments[def.name](args) : args).forEach(
      (argument) => {
        processArguments(
          argument,
          (argDefs) => {
...
async function getCommanderInterface({
  ...
  const generatedMethodDeclarations = allDefs
    .map(({ name, summary, group, complexity, since, argDefs: rawArgDefs }) => {
      // Emit token-specific overloads first so they aren't shadowed by generic variadic ones.
      const tokenCount = (def) =>
        def.filter(({ type }) => /^'[^']*'$/.test(type)).length;

      let argDefs = rawArgDefs
        .slice()
        .sort((a, b) => tokenCount(b) - tokenCount(a));
   ...
   */`;
      ...
      let generatedFunctionDeclarations = flatMap(argDefs, (def) => {
        ...
        if (returnType && typeof returnType === "object") {
          returnType = `RespShape<Resp2<${returnType.resp2}>, Resp3<${returnType.resp3}>, Context>`;
        }
    ...
    methodName.includes("-") ? `['${methodName}']` : methodName
      ...
      return (
        ...
        (overrides[name]
          ? "\n" +
            overrides[name].defs
              .map((line) => "  " + line.replace("$1", name))
              .join("\n")
          : "") +
        (overrides[name] && overrides[name].overwrite
          ? ""
          : generatedFunctionDeclarations.join(""))
      );
    })
    .join("\n");
