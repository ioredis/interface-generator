...
const processArguments = (
  ...
  switch (type) {
    ...
    case "oneof": {
      const overrides = [];
      args.forEach((arg) =>
        processArguments(arg, (processed) => {
...
function processSubcommands(def, allDefs) {
  ...
  for (const command of commands) {
    try {
      ...
      if (result.subcommands) {
        processSubcommands(result, allDefs);
      } else {
        processCommand(result, [], allDefs);
      }
    } catch (err) {
